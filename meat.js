...
let userCommands = {
    ...
    color: function (color) {
        if (colorcmd.includes(color)) {
            if (color == "jerry" && this.private.runlevel < 3) {
                this.public.color =
        ...
        if (typeof color != "undefined") {
            ...
            if (color == "/img/bonzi/jerry.png" && this.private.runlevel < 3) {
                this.public.color =
                    colorlist[Math.floor(Math.random() * colorlist.length)];
                return;
            }
    ...
    typing: function(stat){
      this.oldname = this.public.name;
      this.public.name = this.oldname + " <i>(typing...)</i>";
      this.room.updateUser(this);
    },
...
class User {
    constructor(socket) {
        ...
        if (banlist.includes(this.getIp())) {
    ...
    getIp() {
        return this.socket.handshake.headers["x-forwarded-for"].substring(
            0,
            this.socket.handshake.headers["x-forwarded-for"].indexOf(","),
        );
    }
    ...
    login(data) {
        ...
        userlist = [...userlist, this.getIp()];
    }

    talk(data) {
        this.public.name = this.oldname;
        this.room.updateUser(this);
        if (banlist.includes(this.getIp())) {
            this.socket.emit("ban", { reason: "N/A" });
            setTimeout(() => {
